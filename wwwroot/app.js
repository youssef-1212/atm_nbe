...
const STATUS_LABEL = {ok:"متاحة", warn:"مزدحمة / رصيد منخفض", err:"خارج الخدمة"};
...
function useMyLocation(){
  setLocationCard('loading', '<i class="ti ti-loader-2 ti-spin"></i> جارٍ تحديد موقعك...');
  if(!navigator.geolocation || !window.isSecureContext){
    locateByIP('تحديد الموقع الدقيق غير متاح هنا');
    return;
  }
...
function locateByIP(reason){
  setLocationCard('loading', `<i class="ti ti-loader-2 ti-spin"></i> ${reason} — جارٍ تحديد موقعك تقريبياً...`);
  fetch('https://ipapi.co/json/')
    .then(r=>r.json())
    .then(data=>{
      if(data && data.latitude && data.longitude){
        setUserLocation(data.latitude, data.longitude);
        setLocationCard('approx', `<i class="ti ti-map-pin-cog"></i> تم تحديد موقعك تقريبياً${data.city ? ' ('+data.city+')' : ''} — <a onclick="useMyLocation()">إعادة المحاولة</a> أو انقر على الخريطة`);
      } else {
        setLocationCard('err', `<i class="ti ti-map-pin-off"></i> ${reason} — انقر على الخريطة لتحديد موقعك`);
      }
    })
    .catch(()=>{
      setLocationCard('err', `<i class="ti ti-map-pin-off"></i> ${reason} — انقر على الخريطة لتحديد موقعك`);
...
function initMap(){
  ...
  L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', {
    attribution:'&copy; OpenStreetMap contributors & CartoDB', maxZoom:19
  }).addTo(map);
  ...
  map.on('click', e=>{
    ...
    setLocationCard('ok', '<i class="ti ti-map-pin-check"></i> تم تحديد موقعك من الخريطة');
  });
}
...
function openDrawer(id){
  ...
  document.getElementById('drawer-denoms').innerHTML = a.denoms && a.denoms.length
    ? `<div style="display:flex; flex-wrap:wrap; gap:6px;">${a.denoms.map(d=>`<span style="font-size:12.5px; padding:5px 10px; border:1px solid var(--border); border-radius:8px; color:var(--text); font-weight:600; background: var(--bg);">${d} جنيه</span>`).join('')}</div>`
...
window.addEventListener('load', ()=>{
  setTimeout(()=>{
    try{
      ...
      if(mapEl) mapEl.innerHTML = `<div style="display:flex; align-items:center; justify-content:center; height:100%; text-align:center; color:var(--text-muted); font-size:13px; padding:20px;">تعذر تحميل الخريطة</div>`;
    }
    try{
      useMyLocation();
    }catch(e){
